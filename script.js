...
function solveEquation() {
    ...
    try {
        ...
        // Функция для вычисления числовой части выражения
        function calculateConstant(expr) {
            // Заменяем x на 0 и вычисляем
            let tempExpr = expr.replace(/x/g, '0');
            return eval(tempExpr);
        }
        
        // Функция для получения коэффициента при x
        function getXCoefficient(expr) {
            let coeff = 0;
            
            // Временная замена: заменяем все числа на 0, оставляя только x
            let xExpr = expr.replace(/\d+/g, '0');
            xExpr = xExpr.replace(/\./g, '');
            
            // Обрабатываем выражения со скобками
            if (xExpr.includes('(')) {
                // Для выражений типа (x + a) или (2x + b)
                let matches = expr.match(/([\d.]*)x/g) || [];
                for (let match of matches) {
                    let num = match.replace('x', '');
                    if (num === '' || num === '+') coeff += 1;
                    else if (num === '-') coeff -= 1;
                    else coeff += parseFloat(num);
                }
            } else {
                // Простые случаи без скобок
                ...
                for (let term of terms) {
                    ...
                    if (cleanTerm.includes('x')) {
                        let coeffPart = cleanTerm.replace('x', '');
                        if (coeffPart === '') {
                            coeff += sign * 1;
                        } else if (coeffPart === '-') {
                            coeff -= 1;
                        } else {
                            coeff += sign * parseFloat(coeffPart);
                        }
                    }
                }
            }
        ...
        // Вычисляем константы (числовые части)
        let leftConst = calculateConstant(left);
        let rightConst = calculateConstant(right);
        ...
        console.log('После переноса:', {totalXCoeff, totalConst});
        ...
        // Показываем упрощенное уравнение
        let simplifiedEq = '';
        if (totalXCoeff === 1) {
            simplifiedEq = `x`;
        } else if (totalXCoeff === -1) {
            simplifiedEq = `-x`;
        } else {
            simplifiedEq = `${totalXCoeff}x`;
        }
        
        if (totalConst >= 0) {
            simplifiedEq += ` = ${totalConst}`;
        } else {
            simplifiedEq += ` - ${Math.abs(totalConst)} = 0`;
        }
        
        steps.push(`<strong>2. После упрощения:</strong> ${simplifiedEq}`);
        steps.push(`<strong>3. Решение:</strong> x = ${totalConst} / ${totalXCoeff}`);
        steps.push(`<strong>4. Ответ:</strong> x = ${solution}`);

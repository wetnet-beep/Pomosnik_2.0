    ...
    try {
        ...
        if (equation.includes('x')) {
            ...
            if (eq.includes('=')) {
                ...
                let solution = "Решение: " + equation + " → x = ...";
                
                // Здесь можно добавить более сложную логику решения
                if (left.includes('+')) {
                    ...
                    if (parts[0].includes('x')) {
                        ...
                        solution = `Решение: ${equation} → x = (${rightNum} - ${constant}) / ${coeff} = ${(rightNum - constant) / coeff}`;
                    }
                }
                
                resultDiv.innerHTML = `<div class="success">✅ ${solution}</div>`;
            } else {

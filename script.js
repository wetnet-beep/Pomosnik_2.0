...
function solveEquation() {
    ...
    try {
        ...
        // Упрощаем уравнение: убираем пробелы, заменяем запятые
        let eq = equation.replace(/\s/g, '').replace(/,/g, '.');
        
        // Разделяем на левую и правую части
        let [left, right] = eq.split('=');
        
        // Функция для вычисления числового выражения
        function calculateExpression(expr) {
            // Временная замена для правильного парсинга
            expr = expr.replace(/x/g, '0');
            ...
            try {
                ...
                throw new Error('Невозможно вычислить выражение: ' + expr);
            }
        }
        
        // Функция для упрощения выражения с x
        function simplifyExpression(expr) {
            ...
            // Добавляем + в начало для упрощения парсинга
            expr = expr.replace(/^-/, '0-').replace(/([+-])/g, ' $1 ');
            ...
            for (let token of tokens) {
                if (token === '+') {
                    ...
                    hasX = true;
                    if (token === 'x') {
                        xCoefficient += currentSign * 1;
                    } else {
                        let coeff = token.replace('x', '');
                        if (coeff === '' || coeff === '+') coeff = '1';
                        if (coeff === '-') coeff = '-1';
                        xCoefficient += currentSign * parseFloat(coeff);
                    }
                } else if (!isNaN(parseFloat(token))) {
                    constant += currentSign * parseFloat(token);
                }
            }
        ...
        let leftSimplified = simplifyExpression(left);
        let rightSimplified = simplifyExpression(right);
        
        console.log('Упрощенные части:', leftSimplified, rightSimplified);
        
        // Переносим все x влево, все числа вправо
        let finalXCoefficient = leftSimplified.xCoefficient - rightSimplified.xCoefficient;
        let finalConstant = rightSimplified.constant - leftSimplified.constant;
        
        console.log('После переноса:', finalXCoefficient, finalConstant);
        
        if (finalXCoefficient === 0) {
            if (finalConstant === 0) {
        ...
        let solution = finalConstant / finalXCoefficient;
        
        // Форматируем решение для красивого вывода
        let solutionSteps = [];
        solutionSteps.push(`<strong>Исходное уравнение:</strong> ${equation}`);
        
        if (finalXCoefficient === 1) {
            solutionSteps.push(`<strong>Шаг 1:</strong> x + ${leftSimplified.constant} = ${rightSimplified.constant}`);
            solutionSteps.push(`<strong>Шаг 2:</strong> x = ${rightSimplified.constant} - ${leftSimplified.constant}`);
        } else if (finalXCoefficient === -1) {
            solutionSteps.push(`<strong>Шаг 1:</strong> -x + ${leftSimplified.constant} = ${rightSimplified.constant}`);
            solutionSteps.push(`<strong>Шаг 2:</strong> -x = ${rightSimplified.constant} - ${leftSimplified.constant}`);
            solutionSteps.push(`<strong>Шаг 3:</strong> x = -(${rightSimplified.constant} - ${leftSimplified.constant})`);
        } else {
            solutionSteps.push(`<strong>Шаг 1:</strong> ${finalXCoefficient}x = ${finalConstant}`);
            solutionSteps.push(`<strong>Шаг 2:</strong> x = ${finalConstant} / ${finalXCoefficient}`);
        }
        
        solutionSteps.push(`<strong>Ответ:</strong> x = ${solution}`);
        
        resultDiv.innerHTML = `
            <div class="success">
                <h4>✅ Уравнение решено!</h4>
                <div class="solution-steps">
                    ${solutionSteps.map(step => `<p>${step}</p>`).join('')}
                </div>
            </div>
        `;
